...
try {
  ...
  if (-not $css.Contains('.dream-home') -or -not $css.Contains('.dream-task') -or
    ...
    throw 'The fork-specific Windows renderer overlay lost its Internet Angel marker contract.'
  }
  $macCssPath = Join-Path (Split-Path -Parent $Root) 'macos\assets\dream-skin.css'
  if (-not (Test-Path -LiteralPath $macCssPath)) {
    throw 'The upstream macOS canonical CSS asset is missing.'
  }
  if ((Get-FileHash -Algorithm SHA256 -LiteralPath $macCssPath).Hash -ceq
    (Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $Root 'assets\dream-skin.css')).Hash) {
    throw 'The fork-specific Windows Internet Angel overlay was unexpectedly replaced by the generic runtime.'
  }
  ...
  foreach ($requiredRendererBehavior in @(
    ...
    'MUTATION_REFRESH_INTERVAL_MS', 'MUTATION_MAX_WAIT_MS',
    'SAFETY_REFRESH_INTERVAL_MS', 'runEnsureSafely', 'scheduleGeometry',
    'styleMode: "style"'
  )) {
  ...
  foreach ($forbiddenRendererBehavior in @(
    ...
    if ($rendererSource.Contains($forbiddenRendererBehavior)) {
      throw "Windows renderer restored an unbounded or allocation-heavy behavior: $forbiddenRendererBehavior"
    }
  }
