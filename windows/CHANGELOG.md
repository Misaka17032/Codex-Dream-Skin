...
- 托盘现在会同时核对 injector PID 与进程启动时间，不再把 Codex 退出、闪退或重启后遗留的 `state.json` 误报为“运行中”。
- 选择已保存主题或更换背景图后会自动请求一次串行化安全重应用；若官方 Codex 正在无 CDP 模式运行，仍会先询问是否重启。
- 重应用最多等待 15 秒获取操作锁，避免连续点击主题时启动多个互相竞争的 watcher 或反复重启 Codex。
- 文档明确说明源码安装只部署运行时并启动托盘，首次使用需要通过 `Codex Dream Skin` 快捷方式启动。
